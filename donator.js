...
CryptDonator.prototype = {
    ...
    loadStylesheet: function() {
        if(!in_array(this.theme, CryptDonator_THEMES)) {
            ...
            this.headEl.appendChild(stylesheet);
    ...
    loadLibs: function() {
        ...
        this.headEl.appendChild(vanillalib);
        this.headEl.appendChild(zerolib);   
    },
    ...
    loadConfig: function() {
        ...
        ajaxJson(configUrl, function(responseText) {
            if(typeof(JSON) === "undefined") {
                crypt.config = eval("("+responseText+")");
            } else {
    ...
    changeQr: function(e) {
        ...
        if(typeof(e) === "string") {var coinName = e;}
            ...
            //console.dir(event);
            if(!e.target) {
                var thisTarget = event.srcElement;
                console.dir(thisTarget);
                var thisTargetTag = thisTarget.nodeName;
            } else {
                var thisTarget = e.target;
                var thisTargetTag = thisTarget.tagName;
            }
...
function ajaxJson(page, callback) {
    ...
    request.onreadystatechange = function() {
        if(request.readyState === 4 && request.status===200) {
            var responseText = trim(this.response || this.responseText);
